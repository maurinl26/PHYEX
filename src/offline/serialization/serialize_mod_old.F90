...
contains
  ...
  subroutine serialize( &
   ...
   ! Serialization routine used to generate the input data
    ...
    call fs_add_serializer_metainfo(ppser_serializer, 'LDSLPHY', LDSLPHY)
    call fs_add_serializer_metainfo(ppser_serializer, 'LDMAINCALL', LDMAINCALL)
  ...
  subroutine serialize_ice_adjust( &
    ...
    real(kind=JPRB), intent(in) :: PEXNREF 

  ! #ifdef HAVE_SERIALBOX
    print *, "[ICE3] Serializing input data to Serialbox format"
