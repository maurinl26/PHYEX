MODULE GETDATA_ICE_ADJUST_MOD
    ...
    ! HDF5 file 
    character(len=12), parameter :: filename = "reference.h5"  !File name
    character(len=5), parameter :: dataset_name = "rhodj"      ! dataset name
    
    ! Identifier
    integer(HID_T):: file_id    ! File id
    integer(HID_T):: dset1_id   ! Dataset 1 id
    integer(HID_T):: dspace1_id ! Dataspace identifier

    ! FP array
    integer(HSIZE_T), dimension(2):: dims1 = (/NGPTOT/;/KLEV/)
    real, dimension(NGPTOT, KLEV) :: dset1_data

    ! Misc variables
    integer:: error ! Error flag

    ! Script from getdata_ice_adjust.F90
    INTEGER      :: KLON 
    INTEGER      :: KIDIA  
    INTEGER      :: KFDIA  
    INTEGER      :: KLEV  
    ...
    ! Create a dataset
    call h5dcreate_f(file_id, dataset_name, H5T_NATIVE_REAL, dspace1_id, dset1_id, error)
    
    ! Write dataset
    call h5write_f(dset1_id, H5T_NATIVE_REAL, phrodj, dims1, error)
