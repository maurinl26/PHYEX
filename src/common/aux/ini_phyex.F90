...
!
   ...
   USE MODD_CST, ONLY: SETUP_CST, PRINT_CST, RCST
   USE MODD_PARAM_ICE_n, ONLY: PARAM_ICEN_INIT, PARAM_ICEN, &
   ...
   USE MODD_CLOUDPAR_N, ONLY: CLOUDPARN
   
   USE MODD_PARAM_MFSHALL_N, ONLY: PARAM_MFSHALLN, PARAM_MFSHALLN_INIT, PARAM_MFSHALL_GOTO_MODEL
   USE MODD_TURB_N, ONLY: TURBN, TURBN_INIT, LHARAT
   USE MODD_CTURB, ONLY: CSTURB, CTURB_ASSOCIATE
   USE MODD_NEB_N, ONLY: NEBN, NEBN_INIT, CCONDENS, LSTATNW, LSUBG_COND
   ...
   ! USE MODD_RAIN_ICE_n, ONLY: RAIN_ICE_PARAM_GOTO_MODEL
...
!
   ...
   use mode_ini_cloudpar, only: ini_cloudpar
...
! Debug 
   print *, "debug : ini_phyex.F90 - KFROM", KFROM, "; KTO = ", KTO
!
...
!**       CST
!
   print *, "debug : ini_phyex.F90 - LLINIT", LLINIT
   IF (LLINIT) THEN
      IF (IPRINT == 2) WRITE (UNIT=KLUOUT, FMT='('' MODD_CST '')')
      ! IF (PRESENT(PHYEX_IN)) CST = PHYEX_IN%CST
      print *, "debug : ini_phyex.F90 - start INI_CST"
      CALL setup_cst()
      print *, "debug : ini_phyex.F90 - INI_CST done"
      IF (IPRINT == 2) CALL PRINT_CST(KLUOUT)
      IF (PRESENT(PHYEX_OUT)) PHYEX_OUT%CST = RCST
   END IF
!
!**       MICROPHYSICS SCHEME
!
   IF (CMICRO == 'ICE3' .OR. CMICRO == 'ICE4' .OR. CMICRO == 'LIMA') THEN
      ...
      IF (IPRINT == 2) WRITE (UNIT=KLUOUT, FMT='('' MODD_PARAM_ICEN, &
      &MODD_RAIN_ICE_DESCRN, MODD_RAIN_ICE_PARAMN, MODD_CLOUDPARN '')')

      ! Association vers pointeur
      IF (LLCHANGEMODEL) THEN

         ! Debug 
         print *, "KFROM", KFROM, "; KTO = ", KTO
         
         print *, "debug : ini_cloudpar"
         call ini_cloudpar()

         ! TODO : remove rain_ice_descr_goto_model
         ! TODO : replace with an association routine in ini_rain_ice
      ...
      IF (LLINIT) THEN
         print *, "debug : ini_phyex.F90 - start ini_rain_ice"
         ! TODO : associate routine for rain_ice_descr and rain_ice_paramn
         CALL INI_RAIN_ICE(KLUOUT, PTSTEP, PDZMIN, CLOUDPARN%NSPLITR, CMICRO)
         print *, "debug : ini_phyex.F90 - end ini_rain_ice"
         CALL INI_TIWMX
         RAIN_ICE_PARAMN%TIWMX => TIWMX

         IF (RAIN_ICE_PARAMN%XFRMIN(16) > 0.) THEN
            ! TODO : associate for ini_snow
...
!
   print *, "debug : ini_phyex.F90 - lima_update_nsv skipped "
!    IF (LLINIT) THEN
...
!
   print *, "debug : ini_phyex.F90 - CSCONV = ", CSCONV
   IF (CSCONV == 'EDKF') THEN
      IF (IPRINT == 2) WRITE (UNIT=KLUOUT, FMT='('' MODD_PARAM_MFSHALL_n '')')

      ! TODO : remove goto_model (replace with association in ini_mfshall)
...
!
   ...
   print *, "debug : ini_phyex.F90 - start nebn_init"
   CALL NEBN_INIT(HPROGRAM, TPFILE, LDNEEDNAM, KLUOUT, &
                    &LDDEFAULTVAL, LDREADNAM, LDCHECK, KPRINT)
   print *, "debug : ini_phyex.F90 - end nebn_init"
...
!
   IF (CTURB == 'TKEL') THEN
      ...
      print *, "debug : ini_phyex.F90 - start turbn_init"
      CALL TURBN_INIT(HPROGRAM, TPFILE, LDNEEDNAM, KLUOUT, &
                     &LDDEFAULTVAL, LDREADNAM, LDCHECK, KPRINT)
      print *, "debug : ini_phyex.F90 - end turbn_init"
...
!
   ...
   print *, "debug : ini_phyex.F90 - initialization done"
