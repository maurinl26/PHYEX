...
module serialize_mod
  ...
  contains
    ...
    !   INTEGER(KIND=JPIM),INTENT(IN)       :: KFLDX
      real(kind=JPRB)   ,intent(inout)    :: PTSTEP,
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: PRHODJ(KLON,KLEV)
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: PEXNREF(KLON,KLEV)    
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: PSIGS(KLON,KLEV)    
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: PMFCONV(KLON,KLEV)  
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: PPABMS(KLON,KLEV) 
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: ZZZ(KLON,KLEV)  
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: PCF_MF(KLON,KLEV) 
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: ZRS(KLON,KLEV) 
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: PRS(KLON,KLEV) 
      REAL(KIND=JPRB)   ,INTENT(INOUT)    :: PTHS(KLON,KLEV) 
    ...
    !   call fs_add_serializer_metainfo(ppser_serializer, 'KFLDX', KFLDX)
      call fs_add_serializer_metainfo(ppser_serializer, 'PTSTEP', PTSTEP)
    ...
    !   call fs_add_serializer_metainfo(ppser_serializer, 'YREPHLI_RLPP00', YREPHLI%RLPP00)
      ...
      call fs_write_field(ppser_serializer, ppser_savepoint, 'PRHODJ', PRHODJ)
      call fs_write_field(ppser_serializer, ppser_savepoint, 'PEXNREF', PEXNREF)
      call fs_write_field(ppser_serializer, ppser_savepoint, 'PSIGS', PSIGS)
      call fs_write_field(ppser_serializer, ppser_savepoint, 'PMFCONV', PMFCONV)
      call fs_write_field(ppser_serializer, ppser_savepoint, 'ZZZ', ZZZ)
      call fs_write_field(ppser_serializer, ppser_savepoint, 'PCF_MF', PCF_MF)
      call fs_write_field(ppser_serializer, ppser_savepoint, 'ZRS', ZRS(:, :, :, :))
      call fs_write_field(ppser_serializer, ppser_savepoint, 'PRS', PRS(:, :, :, :))
    ...
    call fs_read_field(ppser_serializer, ppser_savepoint, 'PRHODJ', PRHODJ)
    call fs_read_field(ppser_serializer, ppser_savepoint, 'PEXNREF', PEXNREF)
    call fs_read_field(ppser_serializer, ppser_savepoint, 'PSIGS', PSIGS)
    call fs_read_field(ppser_serializer, ppser_savepoint, 'PMFCONV', PMFCONV)
    call fs_read_field(ppser_serializer, ppser_savepoint, 'ZZZ', ZZZ)
    call fs_read_field(ppser_serializer, ppser_savepoint, 'PCF_MF', PCF_MF)
    call fs_read_field(ppser_serializer, ppser_savepoint, 'ZRS', ZRS(:, :, :, :))
    call fs_read_field(ppser_serializer, ppser_savepoint, 'PRS', PRS(:, :, :, :))
    ...
    subroutine serialize_reference( &
      & KLON, KLEV, PTSTEP&
      & PRHODJ, PEXNREF, PSIGS, PMFCONV,&
      & PPABSM, ZZZ, PCF_MF, ZRS(:, :, :, :),&
      & PRS(:, :, :, :), PTHS &
