...
This module provides a Python/CuPy interface to the Fortran OpenACC routines
through a C bridge defined in phyex_bridge.F90.

GPU Acceleration:
- Requires NVIDIA GPU with OpenACC support
- Uses CuPy for zero-copy GPU array management (device pointers passed in)
...
class IceAdjustGPU:
    ...
    def __call__(self,
        ...
        # Coerce Python scalars to C types *before* releasing the GIL
        # (coercion from Python objects is not allowed inside a nogil block)
        cdef int c_nlon = nlon
        cdef int c_nlev = nlev
        cdef int c_krr = self.krr
        cdef float c_timestep = self.timestep
